	...
	$(document).ready(function () {
		...
		$(document).on("change", "select.country_to_state, input.country_to_state", function () {
			var country = $( this ).val(),
				$statebox = $( this ).closest( '.address-field' ).find( '#billing_state, #shipping_state, #calc_shipping_state' ),
        ...
        $form.submit(function (e) {
            var $this  = $( this ),
				$formRow = $this.find('.validate-required'),
                isValid = false;

            $.each($formRow, function (i, el) {

                var inputValue = $(el).find('input').val();
                var selectValue = $(el).find('select').val();
                var isState = $(el).css('display');

                if(isState !== 'none'){
                    if(inputValue === '' || selectValue === ''){
                        $(el).addClass( 'woocommerce-invalid woocommerce-invalid-required-field' ).removeClass(' woocommerce-validated');
                        e.preventDefault();
                        isValid = true;
                    }else {
                        $(el).removeClass( 'woocommerce-invalid woocommerce-invalid-required-field' ).addClass(' woocommerce-validated');
                    }
                }

            });

			if (isValid) scrollToInputInvalid();
        });

        function scrollToInputInvalid(){
            var linkId = $form.find('.woocommerce-invalid')[0];

            $('html, body').animate({scrollTop:$(linkId).position().top}, 1000, function () {
                jQuery(linkId).find('input').focus();
            });
        }

		/* prohibition of deleting the last form */
        $form.on('click', '.delete', function () {

            if($form.find('.address_block').length === 1){
                return false;
            }

            var self = $(this);
            var $checkboxBlock = self.closest('.address_block').find('.default_shipping_address');
            var $checkboxIndex = $form.find('.address_block');
            var checkboxValue = $checkboxBlock.val();
            var $defaultShippingAddress = $('.default_shipping_address');

            if(checkboxValue === 'true' && $checkboxIndex.length > 2  && $($defaultShippingAddress[0]).val() !== 'true'){
                $($defaultShippingAddress[0]).trigger('click');
            }else if(checkboxValue === 'true' && $checkboxIndex.length > 2){
                $($defaultShippingAddress[1]).trigger('click');
            }else if(checkboxValue === 'true' && $checkboxIndex.length === 2  && $($defaultShippingAddress[0]).val() !== 'true'){
                $($defaultShippingAddress[0]).trigger('click');
            }else if(checkboxValue === 'true' && $checkboxIndex.length === 2){
                $($defaultShippingAddress[1]).trigger('click');
            }
